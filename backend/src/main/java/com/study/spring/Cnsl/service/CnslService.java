...
public class CnslService {
	...
	public Integer reserveCounseling(CnslReqDto cnslReqDto) {
		Member member = memberRepository.findById(cnslReqDto.getMember_id())
				.orElseThrow(() -> new IllegalArgumentException("사용자가 없습니다."));
		Member counselor = memberRepository.findById(cnslReqDto.getCnsler_id())
				.orElseThrow(() -> new IllegalArgumentException("상담사가 없습니다."));
		/*
		 * [에러 처리 해야 될 것] 
		 * stat A 
		 * todo yn Y 
		 * 동일한 사용자와 동일한 상담사의 상담이 끝나지 않았는데 상담을 신청하는 경우 
		 * ㄴ select 해봐야됨 (상담 진행 중인지 확인하는 메서드가 있어야 됨)
		 */
	...
	// 신청 시 밸리데이션 체크 
	public Optional<IsCnslDto> isCounseling(UUID memberId, UUID cnslerId, LocalDate cnsl_date) {
		return cnslRepository.isCounseling(memberId, cnslerId, cnsl_date);
		
	}

	// [상담사 특정 일자 예약 리스트] : 이걸 통해 프론트에서 해당 일자에 상담 가능한 시간을 보여줄 거임
	public List<CnslerDateDto> getAvailableSlotsForCnsler(UUID cnslerId, LocalDate cnslDt) {
	...
	// 상담 수정 + 삭제에서 getMemberId와 로그인한 UUID 불일치 시 삭제 및 수정 못하게 로직 처리 해야 함(아직 로그인 기능이 없어서 구현 못함)	
	// [상담 수정]
	@Transactional
