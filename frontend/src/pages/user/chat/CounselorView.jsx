...
const CounselorView = () => {
  ...
  const counselor = useMemo(
    () => counselors.find((item) => item.id === c_id),
    [c_id],
  );
        ...
        <main className="px-[18px] pt-4 flex flex-col gap-4">
          <div className="bg-white rounded-[14px] overflow-hidden shadow-[0_8px_16px_rgba(0,0,0,0.06)]">
            ...
            <div className="px-4 pb-4 -mt-8">
              ...
              <p className="text-[12px] text-[#6b7280]">
                {counselor.tags.map((tag) => `#${tag}`).join(' ')}
              </p>
          ...
          {reservationDone && (
            <div className="bg-white rounded-[12px] p-3 border border-[#c7d2fe] text-[13px] text-[#1e3a8a]">
              상담 예약이 완료되었습니다. 2영업일 이내 확인 후 처리될
              예정입니다.
            </div>
          )}
            ...
            <div className="grid grid-cols-3 text-[13px] text-[#111827] gap-2">
              <div className="border border-[#dbe3f1] rounded-[10px] p-3 text-center">
                ...
                <p className="font-semibold">
                  {counselor.prices.chat.toLocaleString()}원
                </p>
              </div>
              <div className="border border-[#dbe3f1] rounded-[10px] p-3 text-center">
                <p className="text-[11px] text-[#6b7280]">전화</p>
                <p className="font-semibold">
                  {counselor.prices.call.toLocaleString()}원
                </p>
              </div>
              <div className="border border-[#dbe3f1] rounded-[10px] p-3 text-center">
                <p className="text-[11px] text-[#6b7280]">방문</p>
                <p className="font-semibold">
                  {counselor.prices.visit.toLocaleString()}원
                </p>
              </div>
            </div>
          </section>
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <h1 className="text-4xl font-bold text-gray-800 mb-2">
                    {counselor.name}{' '}
                    <span className="text-gray-600">{counselor.title}</span>
                  </h1>
              ...
              {[1, 2, 3].map((idx) => (
                ...
                >
                  ...
                  <p className="text-sm text-gray-600 mb-2">
                    {`이용자 ${idx}`} |{' '}
                    {new Date(2026, 0, idx).toLocaleDateString('ko-KR')}
                  </p>
                  <p className="text-base text-gray-800 leading-relaxed">
                    정말 도움이 많이 되었습니다. 제가 혼자서는 해결하기 어려웠던
                    고민들을 차근차근 풀어나갈 수 있었습니다. 감사합니다!
                  </p>
                </div>
              ))}
            </div>
